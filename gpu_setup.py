...
GPU_CONFIGS = {
    'NVIDIA GeForce RTX 3090': {
        ...
        'core_clock': None,  # Consumer RTX GPUs don't support clock locking
        'lock_clocks': False,  # Only set power cap, clocks boost dynamically
    },
    ...
    'NVIDIA A30': {
        ...
        'core_clock': 1440,  # MHz
        'lock_clocks': True,  # Datacenter GPUs support -lgc (graphics lock only)
    },
    'NVIDIA A100': {
        'name': 'A100',
...
def set_core_clock(gpu_config):
    """Set fixed core clock on GPU 0 (only for datacenter GPUs)."""
    lock_clocks = gpu_config.get('lock_clocks', False)

    if not lock_clocks:
        print(f"\nSkipping clock locking for {gpu_config['name']}...")
        print("Consumer RTX GPUs don't support clock locking via nvidia-smi.")
        print("Clocks will boost dynamically within power cap constraints.")
        return

    core_clock_mhz = gpu_config['core_clock']
    print(f"\nLocking graphics clock to {core_clock_mhz} MHz on GPU 0...")
    print("(Memory clock will remain at default)")

    # Lock graphics clock using -lgc (does NOT affect memory clock)
    # Setting min=max locks it to a single frequency
    cmd = f"sudo nvidia-smi -i 0 -lgc {core_clock_mhz},{core_clock_mhz}"
    output, ret = run_command(cmd, check=False)
    if ret != 0:
        print(f"Error: Could not lock graphics clock to {core_clock_mhz} MHz")
        print(f"Output: {output}")
        print("\nNote: This GPU may not support clock locking, or the specified")
        print(f"frequency {core_clock_mhz} MHz may not be in the supported range.")
        print("Check supported clocks with: nvidia-smi -q -d SUPPORTED_CLOCKS")
        sys.exit(1)

    print(f"Graphics clock locked to {core_clock_mhz} MHz")
...
def verify_settings(gpu_config):
    ...
    if lock_clocks:
        # For datacenter GPUs with -lgc: check if graphics clock is locked
        cmd = "nvidia-smi --query-gpu=clocks.applications.graphics --format=csv,noheader,nounits -i 0"
        output, ret = run_command(cmd)
        if ret == 0:
            gr_app = output.strip()
            if gr_app != "[N/A]":
                print(f"Graphics Clock (locked): {gr_app} MHz")
            else:
                print(f"Graphics Clock: Not locked (boost mode)")
    else:
        # For consumer RTX GPUs: just show current clocks
        print(f"Graphics Clock: Dynamic (boost mode, constrained by power cap)")
...
def main():
    ...
    if sys.argv[1] == '--detect':
        ...
        if gpu_config.get('lock_clocks'):
            print(f"  Graphics Clock: {gpu_config['core_clock']} MHz (locked via -lgc)")
            print(f"  Memory Clock: Default (not controlled)")
        else:
            print(f"  Graphics Clock: Dynamic boost (not locked)")
            print(f"  Clock Control: Power cap only")
        sys.exit(0)
    ...
    if gpu_config.get('lock_clocks'):
        core_clock = gpu_config['core_clock']
        print(f"Target Graphics Clock: {core_clock} MHz (locked)")
    else:
        print(f"Graphics Clock: Dynamic (power-constrained boost)")
    print(f"{'='*60}\n")
